    ...
    for url in URLS:
        if 'unikonferencje' in url:
            tasks.append(asyncio.create_task(
                scrape_unikon_event_list(url)))
        elif 'eventbrite' in url:
            tasks.append(asyncio.create_task(
                scrape_brite_event_list(url)))
        elif 'crossweb' in url:
            tasks.append(asyncio.create_task(
                scrape_crossweb_articles_event_list(url)))
...
if __name__ == '__main__':
    try:
        with open('last_update_timestamp.txt', 'r') as f:
            ...
            last_update_timestamp = datetime.strptime(last_update_timestamp,
                                                      '%d-%m-%Y %H:%M')
    except FileNotFoundError:
        last_update_timestamp = None  # If the file doesn't exist, set to None

    if (last_update_timestamp is None
            or datetime.now() - last_update_timestamp > timedelta(hours=12)):
        clear_output_dir()
